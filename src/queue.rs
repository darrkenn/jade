...
pub fn create_queue(mp: Sender<MusicPlayer>) -> Sender<Queue> {
    ...
    thread::spawn(move || {
        loop {
            ...
            match recieved {
                Queue::Add(song) => {
                    mp.send(MusicPlayer::AddToQueue(song))
                        .expect("Cant send song to music player thread");
                }
                Queue::End => break,
            }
        }
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            match recieved {
                ...
                VisualQueue::Clear => songs.clear(),
                VisualQueue::SongEnded => {
                    if songs.is_empty() {
                        q.send(Queue::Add(songs.first().unwrap().to_string()))
                            .expect("Cant add song to queue");
                        songs.remove(0);
                    }
                }
                VisualQueue::End => {
